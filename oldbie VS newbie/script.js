...
const result = {
    ...
    "third_grade": { 
        "person": "아무것도 모르는 뉴비가 귀여운 고학년 학교 시설을 통달하신 당신",
...
// click 하면 동작하게 한다
// 첫화면 버튼을 누르면 커버가 없어지고 문제가 나오도록 해라
...
aBtn.addEventListener('click', () => {
    switch (type.innerHTML) {
        ...
        case 'oldbie':
            ...
            // 네, 올드비를 선택하면 올드비 숫자창에 +1 씩 더해준다. 
            oldbie.setAttribute('value', o + 1);
            break;
        // 두번째 뉴비 유형
        case 'newbie':
            let n = parseInt(newbie.value);
            // 네, 뉴비를 선택하면 올드비 숫자창에 +1 씩 더해준다. 
...
// 결과 도출 : 1학년 ~ 5학년 
// 문제의 갯수는 16개니까 4점 1학년, 7점 2학년, 10점 3학년, 13점 4학년, 16 5학년 으로 나누도록 한다
function updateQuestion() { 
    if (num == 16) {
        // 16번 문제가 되었을때 문제는 더이상 보이지 않도록 하고 결과창을 보여주겠다 
        questionContainer.style.display = 'none';
        resultContainer.style.display = 'block';

        // 만약 올드비의 결과값이 4이상이면 첫번째 결과를 불러오겠다
        if(oldbie.value<=4){ result_grade = "first_grade"}
        // 아니면 올드비의 결과값이 4이상 7이하이면 두번째 결과를 불러오겠다
        ...
        // pro : progress bar
        // progress bar 의 길이 css 를 js 에서 조정해준다
